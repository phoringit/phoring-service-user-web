...
class WalletPaymentCard extends StatelessWidget {
  ...
  const WalletPaymentCard({super.key, required this.fromPage}) ;
  ...
  Widget build(BuildContext context) {
    return GetBuilder<CartController>(builder: (cartController){
      ...
      double paidAmount =  CheckoutHelper.calculatePaidAmount(walletBalance: walletBalance, bookingAmount: bookingAmount);
      double remainingWalletBalance = CheckoutHelper.calculateRemainingWalletBalance(walletBalance: walletBalance, bookingAmount: bookingAmount);
          ...
          Padding(padding: const EdgeInsets.all(Dimensions.paddingSizeDefault),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start,children: [
              ...
              Row(children: [
                ...
                Expanded(
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start,children: [
                    Row( children: [
                      Expanded( child: Text(
                        PriceConverter.convertPrice(  cartController.walletPaymentStatus ? paidAmount : walletBalance ),
                        style: robotoBold.copyWith(fontSize: Dimensions.fontSizeExtraLarge,color: Theme.of(context).colorScheme.primary),
                      )),
                      if( cartController.walletPaymentStatus && !isPartialPayment) Text("${'remaining'.tr} ${PriceConverter.convertPrice(remainingWalletBalance)}",
                        style: robotoRegular.copyWith(fontSize: Dimensions.fontSizeSmall,color: Theme.of(context).textTheme.bodyMedium?.color?.withOpacity(0.6)),
                        overflow: TextOverflow.ellipsis,
                      ),
                    ]),
                    const SizedBox(height: Dimensions.paddingSizeExtraSmall,),
                    Text( cartController.walletPaymentStatus && isPartialPayment ? "has_paid_by_your_wallet".tr
                        :  cartController.walletPaymentStatus && !isPartialPayment ? "paid_by_your_account".tr
                        : "you_have_balance_in_your_wallet".tr,
              ...
              Row( mainAxisAlignment: MainAxisAlignment.spaceBetween,children: [
                ...
                cartController.walletPaymentStatus ? Row( children: [
                  ...
                  style: robotoRegular.copyWith(color: Theme.of(context).colorScheme.primary, fontSize: Dimensions.fontSizeDefault),
                ),
                ...
                ) : CustomButton( height: ResponsiveHelper.isDesktop(context) ? 40 : 30 ,width: 80,
                  buttonText: "use".tr,
                  onPressed: (){
