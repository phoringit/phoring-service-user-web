...
class PaymentMethodButton extends StatelessWidget {
  ...
  const PaymentMethodButton({super.key, required this.title, required this.paymentMethodName, required this.assetName, this.hidePaymentMethod = false, this.itemHeight,  this.walletBalance,  this.bookingAmount}) ;

  @override
  Widget build(BuildContext context) {
    return GetBuilder<CheckOutController>(builder: (controller){
      return paymentMethodName == PaymentMethodName.cos ? _CashPaymentView(title, assetName, paymentMethodName, controller, hidePaymentMethod, itemHeight)
      : _WalletPaymentView(title, assetName, paymentMethodName, controller, walletBalance, bookingAmount);
    });
  }
}
...
class _WalletPaymentView extends StatelessWidget {
  ...
  final double? walletBalance;
  final double? bookingAmount;
  const _WalletPaymentView(this.title, this.assetName, this.paymentMethodName, this.controller, this.walletBalance, this.bookingAmount);
    ...
    return GetBuilder<CartController>(builder: (cartController){
      ...
      return Opacity(
        ...
        child: Stack(children: [
          ...
          Column( children: [
            walletCartWidget(context, walletPaymentStatus, remainingWalletBalance, walletBalance, cartController),
          ...
          Column( children: [
            IntrinsicHeight(
              child: Row(children: [
                Expanded(child: walletCartWidget(context, walletPaymentStatus, remainingWalletBalance, walletBalance, cartController)),
            ...
            if(isPartialPayment && walletPaymentStatus) Padding(
              ...
              child: Row(children: [
                if(ResponsiveHelper.isDesktop(context))  const Expanded(child: SizedBox()),
                Expanded(
                  child: Text("pay_the_rest_amount_hint".tr,
  ...
  GestureDetector walletCartWidget(BuildContext context, bool walletPaymentStatus, double remainingWalletBalance, double? walletBalance, CartController cartController) {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          color: ResponsiveHelper.isDesktop(context) ? Theme.of(context).colorScheme.primary.withOpacity(0.02)  : null,
        ),
        child: Center(
          child: Row(children: [
            const SizedBox(width: Dimensions.paddingSizeSmall),
            if(ResponsiveHelper.isDesktop(context))Image.asset(assetName,height: 25,width: 25,),
            ...
            walletPaymentStatus ? Row(children: [
              ...
              child: Container(
                decoration: BoxDecoration(
                  ...
                  color: ResponsiveHelper.isDesktop(context) ? Theme.of(context).colorScheme.primary.withOpacity(0.02)  : Theme.of(context).cardColor,
...
class _CashPaymentView extends StatelessWidget {
  ...
  final bool hidePaymentMethod;
  final double? itemHeight;
  const _CashPaymentView(this.title, this.assetName, this.paymentMethodName, this.controller, this.hidePaymentMethod, this.itemHeight);
      ...
      Opacity(
        ...
        child: GestureDetector(
          ...
          child: Container(
            ...
            height: itemHeight,
            width: ResponsiveHelper.isDesktop(context) ? Dimensions.webMaxWidth / 2 - 35 : null,
            child: Row(children: [
